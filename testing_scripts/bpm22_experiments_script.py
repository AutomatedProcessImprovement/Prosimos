...
from pix_framework.discovery.resource_calendar_and_performance.crisp.resource_calendar import build_full_time_calendar
...
from testing_scripts.bpm_2022_testing_files import canonical_json, experiment_logs, out_folder, process_files
...
def discover_from_xes_log(model_name):
    ...
    [
        ...
        process_files[model_name]["xes_log"],
        process_files[model_name]["bpmn"],
        process_files[model_name]["json"],
        granule,
        conf,
    ...
    [c1_res_profiles, c1_task_distr, c1_calendars] = create_single_pool_model(
        task_resource_events, granule, id_from_name, conf, supp, part, adj_calendar
    )
    save_json_file(
        "%s%s_c_1.json" % (out_folder, model_name),
        c1_res_profiles,
        arrival_time_dist,
        json_arrival_calendar,
        gateways_branching,
    ...
    save_json_file(
        "%s%s_c_2.json" % (out_folder, model_name),
        aggregated_res_profiles,
        arrival_time_dist,
        json_arrival_calendar,
        gateways_branching,
    ...
    save_json_file(
        "%s%s_c_3.json" % (out_folder, model_name),
        aggregated_res_profiles,
        arrival_time_dist,
        json_arrival_calendar,
        gateways_branching,
    ...
    save_json_file(
        "%s%s_c_4.json" % (out_folder, model_name),
        aggregated_res_profiles,
        arrival_time_dist,
        json_arrival_calendar,
        gateways_branching,
    ...
    for json_path in json_paths:
        ...
        hour_emd_index, day_emd_index, emd_trace = compute_median_simulation_emd(
            model_name, p_cases, bpmn_path, json_path, real_log, sim_log_path
        )
...
def create_single_pool_model(task_resource_events, granule_size, id_from_name, min_conf, min_supp, min_part, adj_cal):
    ...
    for t_name in task_resource_events:
        ...
        joint_res_profiles[t_id] = {"name": t_name, "resource_list": resource_list}
...
def create_naive_resource_profiles(
    task_resource_events, observed_task_resources, min_max_task_duration, id_from_name, adj_c
):
    res_profiles = dict()
    res_calendars = dict()
    ...
    for t_name in task_resource_events:
        ...
        for r_obs in observed_task_resources[t_name]:
            ...
            naive_task_dist[t_id] = create_default_distribution(
                min_max_task_duration[t_name][0], min_max_task_duration[t_name][1]
            )
            task_res_distribution[t_id][r_name] = naive_task_dist[t_id]
            i += 1
        res_profiles[t_id] = {"name": t_name, "resource_list": resource_list}
...
def build_profile_entry(res_name):
    return {"id": res_name, "name": res_name, "cost_per_hour": 1, "amount": 1}
...
def build_non_diff(
    ...
    from_name,
    with_fit_c,
    discovered_task_res_dist,
    discovered_pools_json,
    discovered_res_calendars,
):
    ...
    for p_id in res_calendars:
        for t_name in shared_pools[p_id]:
            ...
            t_pool_distr = discover_aggregated_task_distributions(
                task_pools_events[t_name][p_id], with_fit_c, res_calendars[p_id]
            )

            if t_id not in aggregated_res_profiles:
                aggregated_res_profiles[t_id] = {"name": t_name, "resource_list": list()}
            ...
            for r_name in resource_pools[p_id]:
                ...
                aggregated_res_profiles[t_id]["resource_list"].append(
                    {"id": r_name, "name": r_name, "cost_per_hour": 1, "amount": 1}
                )
        ...
        if t_id not in aggregated_res_profiles:
            ...
            for r_info in aggregated_res_profiles[t_id]["resource_list"]:
                ...
                if r_name not in diff_res_calendar:
                    if (
                        t_id in discovered_res_calendars in discovered_res_calendars
                        and r_name in discovered_res_calendars[t_id]
                    ):
