...
class EventAttribute():
    def __init__(self, name, event_atrr_type, value):
        self.name: str = name
        self.event_atrr_type: EVENT_ATTR_TYPE = EVENT_ATTR_TYPE(event_atrr_type)

        if self.event_atrr_type == EVENT_ATTR_TYPE.DISCRETE:
            self.value = parse_discrete_value(value)
        elif self.event_atrr_type == EVENT_ATTR_TYPE.CONTINUOUS:
    ...
    def validate(self):
        if self.event_atrr_type == EVENT_ATTR_TYPE.DISCRETE:
            ...
            if actual_sum_probabilities != 1:
                raise InvalidEventAttributeException(f"Event attribute ${self.name}: probabilities' sum should be equal to 1") 
